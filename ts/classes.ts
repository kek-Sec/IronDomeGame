...
export class Rocket {
    ...
    life: number;
    id: number;
    constructor(startX: number | undefined, startY: number | undefined, targetVx: number | undefined, targetVy: number | undefined, width: number, sizeMultiplier = 1, speedMultiplier = 1) {
    ...
    draw(ctx: CanvasRenderingContext2D) { this._drawTrail(ctx); this._drawHead(ctx); }
...
export class StealthRocket extends Rocket {
    ...
    draw(ctx: CanvasRenderingContext2D) { if (this.isVisible) { super.draw(ctx); } }
}

// A small, fast-moving projectile spawned by a Swarmer
export class Drone extends Rocket {
    constructor(startX: number, startY: number, targetVx: number, targetVy: number, width: number, speedMultiplier = 1) {
...
export class AutomatedTurret {
    ...
    update(rockets: Rocket[]) {
        ...
        const newTracers: TracerRound[] = []; const target = this.findTarget(rockets);
        if (target) {
