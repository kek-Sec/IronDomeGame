...
export class City {
    constructor(x, y, w, h) {
        ...
        // Assign a random structure type for visual variety
        this.structureType = Math.floor(random(0, 3)); // 0: Bunker, 1: Dome, 2: Comms Tower
    }
    ...
    drawCommsTower(ctx) {
        ...
        ctx.fill();
        if (Math.random() > 0.5) { // Blinking light
    ...
    drawRubble(ctx, height) {
        ...
        switch(this.structureType) {
            case 1: // Shattered Dome rubble
                ...
                break;
            default: // Generic/Tower/Bunker rubble
                ctx.moveTo(this.x, height);
...
export class Rocket {
    constructor(startX, startY, targetVx, targetVy, width, sizeMultiplier = 1) {
        ...
        this.radius = 4 * sizeMultiplier; // Apply size multiplier
        this.trail = [];
        this.type = 'standard';
        this.color = 'red';
    ...
    _drawTrail(ctx) {
        ...
        ctx.lineWidth = 2 * (this.radius / 4); // Scale trail with missile size
        ctx.stroke();
    }
    _drawHead(ctx) {
        ctx.beginPath();
...
export class MirvRocket extends Rocket {
    constructor(width, height, sizeMultiplier = 1) {
        ...
        this.width = width;
        this.type = 'mirv';
        this.radius = 7 * sizeMultiplier; // Apply size multiplier
        this.color = 'magenta';
    ...
    split() {
        const childRockets = [];
        const childCount = 3;
        const childSizeMultiplier = (this.radius / 7); // Children inherit parent's size multiplier
